...
export async function createEquipo(req, res) {
  ...
  if (!nombre || !juego) {
    return res.status(400).json({
      error: 'Nombre y juego son obligatorios'
    });
  }
  ...
  // invalidar cache
  await redis.del('cache:/api/equipos');
  await redis.del('cache:/api/equipos/ranking');
...
export async function getEquipo(req, res) {
  console.log('🔵 EQUIPO DESDE SUPABASE');
...
export async function updateEquipo(req, res) {
  ...
  // invalidar cache
  await redis.del(`cache:/api/equipos/${id}`);
  await redis.del('cache:/api/equipos');
  await redis.del('cache:/api/equipos/ranking');
...
export async function rankingEquipos(req, res) {
  console.log('🔵 RANKING DESDE SUPABASE');
